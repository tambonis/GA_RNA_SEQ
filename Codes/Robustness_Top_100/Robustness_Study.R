...
total.repetion <- 100 #Total number of repetions. 
...
for (i in seq(1:length(k.sampled))){#Number of experiments that will be selected.
  ...
  for (j in seq(1:total.repetion)){#Number of repetions.
    ...
    ## GA
    ##############
    r.edger <- list(runedgeR(PM.dat=PM.dat, samples = samples)) #edgeR results. 
    ...
    GA_results <- GA_results[order(GA_results, decreasing = TRUE), ]
    top.GA <- GA_results[1:k.topgenes]
    top.GA <- names(top.GA)
